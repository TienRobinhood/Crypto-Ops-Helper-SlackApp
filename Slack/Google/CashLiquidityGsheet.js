...
export async function copyAndModifySheetTwoDays(newSheetName, lines) {
    try {
        // Step 1: Check if a sheet with the newSheetName already exists
        const spreadsheet = await sheets.spreadsheets.get({
            spreadsheetId: SPREADSHEET_ID,
        ...
        // Step 9: Modify B25 to say "2 day" and change B26 formula to "=N5/10^6"
        requests.push({
            updateCells: {
                rows: [{
                    values: [{
                        userEnteredValue: { stringValue: "2 day" }
                    }]
                }],
                fields: 'userEnteredValue',
                start: {
                    sheetId: newSheetId,
                    rowIndex: 24, // B25 (Row index 24 because it's 0-based)
                    columnIndex: 1 // B25 (Column index 1 because it's 0-based)
                }
            }
        });
        requests.push({
            updateCells: {
                rows: [{
                    values: [{
                        userEnteredValue: { formulaValue: "=N5/10^6" }
                    }]
                }],
                fields: 'userEnteredValue',
                start: {
                    sheetId: newSheetId,
                    rowIndex: 25, // B26 (Row index 25 because it's 0-based)
                    columnIndex: 1 // B26 (Column index 1 because it's 0-based)
                }
            }
        });
        ...
        requests.push({
            updateCells: {
                rows: [{
                    values: [{
                        userEnteredValue: { formulaValue: "=O5/10^6" }
                    }]
                }],
                fields: 'userEnteredValue',
                start: {
                    sheetId: newSheetId,
                    rowIndex: 26, // B26 (Row index 25 because it's 0-based)
                    columnIndex: 1 // B26 (Column index 1 because it's 0-based)
                }
            }
        });
        ...
        requests.push({
            updateCells: {
                rows: [{
                    values: [{
                        userEnteredValue: { formulaValue: "=P5/10^6" }
                    }]
                }],
                fields: 'userEnteredValue',
                start: {
                    sheetId: newSheetId,
                    rowIndex: 27, // B26 (Row index 25 because it's 0-based)
                    columnIndex: 1 // B26 (Column index 1 because it's 0-based)
                }
            }
        });
        ...
        requests.push({
            updateCells: {
                rows: [{
                    values: [{
                        userEnteredValue: { formulaValue: "=Q5/10^6" }
                    }]
                }],
                fields: 'userEnteredValue',
                start: {
                    sheetId: newSheetId,
                    rowIndex: 28, // B26 (Row index 25 because it's 0-based)
                    columnIndex: 1 // B26 (Column index 1 because it's 0-based)
                }
            }
        });
